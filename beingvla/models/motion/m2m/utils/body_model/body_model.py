...
class BodyModel(nn.Module):
    ...
    def __init__(self,
        ...
        try:
            ...
            shapedirs = np.asarray(shapedirs)
            self.comp_register('shapedirs', torch.tensor(shapedirs, dtype=dtype), persistent=persistant_buffer)
        ...
        try:
            ...
            self.comp_register('J_regressor', torch.tensor(smpl_dict['J_regressor'].toarray(), dtype=dtype), persistent=persistant_buffer)
    ...
    def forward(self, root_orient=None, pose_body=None, pose_hand=None, pose_jaw=None, pose_eye=None, betas=None,
        ...
        if pose_hand.device!=pose_body.device:
            pose_hand = pose_hand.to(pose_body.device)

        if trans is None: trans = self.init_trans.expand(batch_size, -1)
        if v_template is None: v_template = self.init_v_template.expand(batch_size, -1,-1).to(pose_body.device)
        if betas is None: betas = self.init_betas.expand(batch_size, -1).to(pose_body.device)
        ...
        if self.use_dmpl:
            ...
            shapedirs = self.shapedirs.to(pose_body.device)
        
        verts, Jtr = lbs(betas=shape_components, pose=full_pose, v_template=v_template,
                            shapedirs=shapedirs, 
                            posedirs=self.posedirs.to(pose_body.device),
                            J_regressor=self.J_regressor.to(pose_body.device), 
                            parents=self.kintree_table[0].long(),
                            lbs_weights=self.weights.to(pose_body.device), 
                            joints=joints, v_shaped=v_shaped,
                            dtype=self.dtype)
