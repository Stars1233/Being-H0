...
class BaseVLMAdapter(ABC, nn.Module):
    ...
    def get_state_dict_key_mapping(self) -> Dict[str, str]:
        """
        Get the mapping from legacy checkpoint keys to BeingVLA format keys.
        
        This method allows each VLM adapter to define how to convert its legacy
        checkpoint format to the standardized BeingVLA format.
        
        Returns:
            Dictionary mapping old keys to new keys. For example:
            {
    ...
    def detect_checkpoint_format(self, state_dict: Dict[str, Any]) -> str:
        """
        Detect the format of a checkpoint state dict.
        
        Each VLM adapter can override this to detect its own checkpoint formats.
        
        Args:
            state_dict: The state dict to analyze
            
        Returns:
            String identifier for the checkpoint format (e.g., 'internvl', 'beingvla', 'llava')
        """
        # Default: check if it's already in BeingVLA format
        if any(k.startswith('vlm_adapter.') for k in state_dict.keys()):
            return 'beingvla'
        return 'unknown'
    
    def convert_legacy_config(self, legacy_config: Any) -> Dict[str, Any]:
        """
        Convert a legacy VLM-specific config to BeingVLA config format.
        
        Each VLM adapter can override this to handle conversion from its legacy
        config format to the standardized BeingVLA config format.
        
        Args:
            legacy_config: The legacy configuration object
            
        Returns:
            Dictionary with BeingVLA config parameters
        """
        raise NotImplementedError(
            f"{self.__class__.__name__} does not support legacy config conversion"
        )
    ...
    def preprocess_conversation(
        ...
        **kwargs
        ...
        """
        Preprocess conversations according to the VLM's specific format.
        
        Each VLM adapter can override this to handle its own conversation templates
        and preprocessing requirements.
        
        Args:
            sources: List of conversation dictionaries
            tokenizer: The tokenizer to use
            **kwargs: Additional VLM-specific parameters
            
        Returns:
            Preprocessed data dictionary
        """
        raise NotImplementedError(
            f"{self.__class__.__name__} does not implement conversation preprocessing"
        )
