...
class BeingHPolicy(BasePolicy):
    ...
    def get_action(self, observations: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        # Lightweight clients may send camera observations as encoded image
        # bytes instead of numpy arrays. OpenCV decodes color images as BGR, so
        # convert them back to RGB before adding the batch dimension expected by
        # the existing policy path: (1, H, W, 3).
        import cv2
        import numpy as np
        for key, value in list(observations.items()):
            if isinstance(value, bytes):
                ...
                if img_bgr is None:
                    raise ValueError(f"Failed to decode image bytes for observation key: {key}")
                img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
                observations[key] = np.expand_dims(img_rgb, axis=0)
